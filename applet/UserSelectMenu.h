...
public:
    ...
    void beginForPad(uint8_t padIndexIn) {
        ...
        // Pre-select the currently bound user for this controller (if any).
        // This makes "open user select from menu" land on the current user.
        const int8_t bound = UserProfiles::padUserIndex(targetPad);
        if (mode == MODE_LIST && bound >= 0 && (uint8_t)bound < UserProfiles::userCount()) {
            list.selectedActual = (int)bound;
        } else {
